...
class RandomCrop(object):
    ...
    def __call__(self, img, img2, label):  #crop the same area of ori-image and label 
        """
        Args:
            img (PIL Image): Image to be cropped.
        Returns:
        ...
        return crop(img, i, j, h, w), crop(img2, i, j, h, w), crop(label, i, j, h, w)
...
class RandomFlip(object):
    ...
    def __call__(self, img, img2,label):
        """
        Args:
...
class CenterCrop(object):
    ...
    def __call__(self, img,img2, label):  
        ...
        i = int(round((h - th) / 2.))
        j = int(round((w - tw) / 2.))
        return crop(img, j,i, tw, th), crop(img2,j,i,tw,th),crop(label, j,i, tw, th)
...
class RandomRotate(object):
    ...
    def __call__(self, img, img2, mask):
        ...
        return img.rotate(rotate_degree, Image.BILINEAR), img2.rotate(rotate_degree, Image.BILINEAR), mask.rotate(rotate_degree, Image.NEAREST)
